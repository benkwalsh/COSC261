...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        '''
        while self.current_char_index < len(self.input_string) and\
            self.input_string[self.current_char_index].isspace():
            self.current_char_index += 1
           
        return self.input_string[self.current_char_index:] 
    ...
    def get_token(self):
        ...
        # if the input contains arbitrary characters
    ...
    def consume(self, *expected_tokens):
        ...
        if self.current_token[0] in expected_tokens:
            ...
            self.current_token = self.get_token()
            if (token[0] is Token.NUM) or (token[0] is Token.ID):
                return token
            else:
                return token[0]
        else:
...
# true_code/false_code(label) jumps to label if the condition is/isn't true.
...
class If_Else_AST:
    def __init__(self, condition, then, body):
        self.condition = condition 
        self.then = then
        self.body = body 
    def __repr__(self):
...
def write():
    ...
    result = expression()
    return Write_AST(result)

def read():
    scanner.consume(Token.READ)
    result = identifier()
    return Read_AST(result)
...
def if_statement():
    ...
    if scanner.lookahead() == Token.ELSE:
        ...
        return If_Else_AST(condition, then, body)
    scanner.consume(Token.END)
...
def booleanfactor():
    if scanner.lookahead() == Token.NOT:
        ...
        return BooleanFactor_AST(booleanfactor(), op, None)
    else:
        result = comparison()
        return result
